...
$contasshpost=anti_sql_injection($_POST['contassh']);
...
$buscaaloc = "SELECT * FROM usuario_ssh where id_usuario_ssh='".$contasshpost."' and id_usuario='".$idconta."'";
$buscaaloc = $conn->prepare($buscaaloc);
$buscaaloc->execute();

if($buscaaloc->rowCount()==0){
        echo '<script type="text/javascript">';
		echo 	'alert("SSH Não encontrada!");';
		echo	'window.location="../../home.php?page=ssh/contas";';
		echo '</script>';
		exit;
}
...
//Insere fatura
$faturains = "insert into fatura_clientes (usuario_id,id_mestre,conta_id,tipo,qtd,data,datavencimento,status,descrição,valor,desconto) values ('".$ssh['id_usuario']."','".$_SESSION['usuarioID']."','".$alocado['id_usuario_ssh']."','".$tipo."','1','".$data."','".$dataexpire."','pendente','".$desc."','".$valorssh."','".$desconto."')";
